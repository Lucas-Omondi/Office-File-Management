...
const login = async () => {
  ...
  if (success) {
    ...
    router.push("/dashboard");  // ✅ Ensured this redirects properly
  } else {
...
<template>
  <div class="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-blue-800 to-indigo-700">
    <div
        v-motion
        :initial="{ opacity: 0, y: -20 }"
        :animate="{ opacity: 1, y: 0 }"
        :transition="{ duration: 0.6, easing: 'easeOut' }"
        class="w-full max-w-md p-8 bg-white/20 backdrop-blur-md shadow-2xl rounded-2xl border border-white/30"
    >
